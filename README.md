...
1. What problem does the context API help solve?
   Gives access to state in any of the components you need it without having the nightmare that is redux. It's a much simplier than redux
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
   Actions are payloads of information that send data from your app to your store
   Reducers put simply take 2 things and return 1. In redux, reducers are a function that determines the changes to an apps state
   The store holds the state tree of your app. You can change state by dispatching an action to it
3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
   App state is global (in terms of the app) and component state is local to the component.
4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
   Redux thunk is middleware that lets your call action creators that return a function instead of an action object.
5. What is your favorite state management system you've learned and this sprint? Please explain why!
   Context API - simplicity and ease of use
